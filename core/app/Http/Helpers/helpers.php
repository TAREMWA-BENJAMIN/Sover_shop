...
function getImage($image, $size = null, $avater = false)
{
    $clean = '';
...
{
    return getAmount($price - (($discount / 100) * $price));
}

function orderTrack()
{
    $lastOrder = Order::latest()->first(['order_track']);
    if ($lastOrder) {
        $lastNum = intval(str_replace('ORD-', '', $lastOrder->order_track));
        $track   = $lastNum + 1;
    } else {
        $track = 1001;
    }
