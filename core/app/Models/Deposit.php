    ...
    public function statusBadgeStatus(): Attribute
    {
        return new Attribute(function(){
            ...
            if($this->status == Status::PAYMENT_SUCCESS){
                $html = '<span class="badge bg--success badge--sm">'.trans('Complete').'</span>';
            }
            elseif($this->status == Status::PAYMENT_PENDING){
                $html = '<span><span class="badge bg--warning badge--sm">'.trans('Pending').'</span>';
            }
            elseif($this->status == Status::PAYMENT_INITIATE ){
                $html = '<span class="badge bg--dark badge--sm">'.trans('Succeed').'</span>';
            }
            else{
                $html = '<span class="badge bg--danger badge--sm">'.trans('Initiated').'</span>';
            }
            return $html;
        });
    }
