...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $order['pending']    = Order::where('status', Status::ORDER_PENDING)->count();
        $order['delivered']  = Order::where('status', Status::ORDER_COMPLETED)->count();
        $order['processing'] = Order::where('status', Status::ORDER_PROCESSING)->count();
        $order['cancel']     = Order::where('status', Status::ORDER_CANCELLED)->count();

        $otherData['category'] = Category::count();
        $otherData['product']  = Product::count();
        $otherData['brand']    = Brand::count();
        $otherData['ticket']   = SupportTicket::count();

        return view('admin.dashboard', compact('pageTitle', 'order', 'otherData', 'widget', 'chart', 'deposit'));
    }
